...
| Déjà-vu et répétition pénalisés | principes de sélectivité | items déjà montrés exclus, comptes des 20 cartes récentes par source | `test_seen_and_repeated_sources_are_penalized_*` |
...
| Authentification et espace personnel | mission Web App | dérivé scrypt, sessions serveur hashées, cookie strict, CSRF, rate-limit, trusted hosts | `tests/test_web_app.py`, smoke HTTPS réel |
| Secret récupérable hors Git | mission Web App | bootstrap no-clobber, fichiers 700/600 sous `~/.config/scout` | `tests/test_web_credentials.py`, `stat` de production |
...
| Service production observable | mission déploiement | Gunicorn, healthcheck, restart policy, révision exacte, logs bornés | build CI, health Docker, `/healthz`, inspect `restarts=0` |
